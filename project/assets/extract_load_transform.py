...
from assets.source_extractor import (
    SqlExtractParser,
...
def extract_load_from_source(
    ...
    for asset in template_env.list_templates():
        sql_extract_parser = SqlExtractParser(
            file_path= asset,
            environment= template_env
        )
        source_database_extractor = DatabaseTableExtractor(
        ...
        table_schema, metadata = source_database_extractor.get_table_schema()
        table_data = source_database_extractor.extract()
        target_postgresql_client.upsert(
